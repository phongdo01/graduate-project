...
// update quantity product after admin approve order success
function updateQuantityProduct(){
    // nhieu order co cung id ( tuong ung voi nhieu product)
    $options = [
        'where' => "OrderId = ".intval($_POST['oid'])
    ];
    $orderDetails = get_all("order_detail", $options);
    // cap nhat lai so luong cua nhung san pham trong don hang
    foreach ($orderDetails as $detail ) {
        $idProduct = $detail['ProductId'];
        $quantity = $detail['Quantity'];
        $sql = "update product set quantity = quantity - ".$quantity." where Id =".$idProduct;
        // echo $sql;die();
        mysql_query($sql) or die(mysql_error());
    }
    // cap nhat nguoi duyet

    $updateUser = "update orders set accepter = ".intval($_SESSION['user']['Id'])." where Id = ".intval($_POST['oid']);
