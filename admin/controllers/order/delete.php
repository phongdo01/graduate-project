...
function deleteOrder() {
	$sql = 'update orders set Status = 2, accepter = '.$_SESSION['user']['Id'];
	$sql = $sql.', ProcessingTime = now()';
	$sql = $sql.' where Id = '.$_GET['oid'];
...
if ($_GET['org']=='process') {
	...
	header('location:admin.php?controller=order&action=index');
}